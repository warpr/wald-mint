...
(function (factory) {
    ...
    class Minter {
        constructor (config) {
            ...
            this.setValue = nodefn.lift (this._client.set.bind (this._client));
        }
        _key (entity) {
            return entity === 'bnode'
        ...
        reset (entity, value) {
            ...
            //
            // this function resets the counter for an entity, which is only useful
            // in unit or integration tests

            return this.setValue (this._key (entity), value ? value : 0);
        }
        newEntity (entity) {
        ...
        bnode () {
            return this.newEntity ('bnode');
        }
        newId (node, datastore) {
            const triples = datastore.find (node, a);

            for (let i = 0; i < triples.length; i++) {
                const triple = triples[i];
                if (this._config.types.hasOwnProperty (triple.object)) {
                    const entity = this._config.types[triple.object];
